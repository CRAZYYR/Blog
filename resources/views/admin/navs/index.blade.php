...
@section('content')
    ...
    <table class="list_tab">
        ...
        <tr>
            ...
            <td class="tc">
               <input onchange="changerOrder(this)" type="text" name="ord[]" value="{{$v['nav_order']}}">
            </td>
            ...
            <td ><a  href="{{$v->link_url}}">{{$v->nav_url}}</a></td>
...
var url="{{url('admin/navs/changerOrder')}}";
function changerOrder(object){
  var wid=$(object).parent().next().html();
  var xid=$(object).val();
    $.post(url,{'nav_order':xid,'nav_id':wid,'_token':'{{csrf_token()}}'},function(data){
        if (data.status) {
